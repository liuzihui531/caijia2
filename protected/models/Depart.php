...
class Depart extends CActiveRecord {
    ...
    public static $cacheKey = "depart";

    //获取部门
    public static function getDepart($key = "") {
        $return = RedisD::get(self::$cacheKey);
        if(!$return){
            $return = self::setDepart();
        }
        if ($key !== "") {
    ...
    public static function setDepart(){
        $model = Yii::app()->db->createCommand("select id,name from depart")->queryAll();
